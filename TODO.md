...
- [x] **Webcam / screenshare over the Cloudflare SFU** — backend + client contract (`docs/voice-client.md`). Every SFU call is proxied (`voice:sfu:publish|pull|renegotiate|close|layer`), so the app secret never leaves the server; pulls resolve through presence. Video always promotes the room (no video mesh); a join past `meshMax` promotes instead of refusing, capped at `VOICE_SFU_MAX`. Publish offers are checked read-only for VP8/Opus and the exact simulcast ladder per `contentHint`. Off unless `CF_REALTIME_APP_ID`/`SECRET` are set.
- [ ] **SFU client in `agree-app`** — the contract is in `docs/voice-client.md` § Fase 2. Until it lands, leave `CF_REALTIME_*` unset in production: the current client only speaks mesh and would be refused on `voice:signal` in a promoted room.
