...
**Voice is signaling only — media never touches this server.** Cloud Run carries no UDP, so a self-hosted SFU is impossible: a room starts as a browser↔browser P2P mesh (audio only) and moves onto the Cloudflare Realtime SFU once it outgrows the mesh or anyone publishes video. `VoiceGateway` (namespace `voice`) carries presence, relays mesh SDP/ICE, hands out TURN credentials, and proxies every SFU call. Design rationale lives in `docs/voice-webrtc.md`, the client contract in `docs/voice-client.md`.
...
- **Topology promotion is one-way.** `VoiceTopologyService` promotes a room to `sfu` past `meshMax` or on the first video publish, and only clears it when the room empties (`release`), so a room oscillating at the threshold can't renegotiate on every join. Deciding (`needsSfu`) and promoting (`promote`) are separate so a refused join or a publish Cloudflare rejects never leaves a room marked promoted. Video never runs on the mesh — adding it would make existing peers renegotiate, breaking the newcomer-offers rule.
- **The SFU is config-gated and proxied.** It is off unless `CF_REALTIME_APP_ID` and `CF_REALTIME_APP_SECRET` are both set — without them the module is exactly Phase 1 (audio mesh, room full at `meshMax`, `ack.video === null`). The app secret lives only in `CloudflareSfuClient`; a client holding it could pull any track in the app, so it must never reach one. A socket's Cloudflare `sessionId` is stored in presence *beside* the participant (`VoiceSfuState`), never on the broadcast `VoiceParticipant`. Pulls are addressed by `{ userId, trackName }` and resolved through presence — the same "presence is the address" rule as `voice:signal`.
- **Egress is the one cost the server controls, so `VoiceSfuService` polices it**: one pull per publisher track, only on a layer the track offers (cheapest by default), `sfuMax` per room. Send bitrate cannot be enforced (`maxBitrate` never reaches the SDP); codecs can. Publish offers are inspected read-only in `voice.sdp.ts` — VP8/Opus only, simulcast rids exactly the source's ladder from `voice.simulcast.ts` — and never rewritten; mesh SDP stays opaque.
- **`voice:sfu:close` takes no offer and never renegotiates** (`force: true` to Cloudflare, and the client retires the transceiver with `replaceTrack(null)` + `direction = 'inactive'` instead of `stop()`). A client offer with a port-0 m-section — what `stop()` generates — frees the mid for Cloudflare to recycle on its next offer with renumbered `a=extmap` ids, and Chrome refuses that remap for the life of the `RTCPeerConnection` (`RTP extension ID reassignment not supported`), poisoning every later negotiation including `createOffer`. So the invariant is **nobody ever offers port 0**; the dead m-section it leaves behind is the price. Rationale in `docs/voice-webrtc.md`.
